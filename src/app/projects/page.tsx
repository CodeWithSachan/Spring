...
export default function ProjectsPage() {
  ...
  // ✅ Map static data → UI type expected by ProjectCard
  const uiProjects: ProjectCardType[] = visibleProjects.map((p) => ({
    id: p.id,
    slug: p.slug,
    title: p.title,
    ...
    maintainerRole: p.maintainer.role,
  }));
      ...
      <div className="relative mx-auto max-w-7xl space-y-14">
        ...
        <div className="flex justify-center">
          <input
            ...
            className="w-full max-w-xl rounded-xl bg-white/[0.06] border border-white/10 px-5 py-3 text-sm text-white"
          />
        </div>
        ...
        <ProjectGrid projects={uiProjects} />
      </div>
    </section>
  );
}